...
$Id_Secretario = isset($_GET['Id_Secretario']) ? $_GET['Id_Secretario'] : (isset($_POST['Id_Secretario']) ? $_POST['Id_Secretario'] : '');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($Res_Pesquisar_Compras)) {
        if ($Res_Pesquisar_Compras !== false && $Res_Pesquisar_Compras->num_rows > 0) {
            ...
            while ($row = $Res_Pesquisar_Compras->fetch_assoc()) {
                ...
                echo "<td class='view-td'><a class='view' href='VisualizarCompra.php?Id=" . $row['Id'] . "&Id_Secretario=" . $Id_Secretario . "'>Visualizar</a></td>";
                echo "</tr>";
            }
            echo "</table>";
        } elseif ($searchError) { 
